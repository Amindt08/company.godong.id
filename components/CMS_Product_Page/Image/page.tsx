...
const GetImageById: React.FC<Props> = ({ id }) => {
    ...
    const fetchDataById = async (id: string) => {
        try {
            const response = await axios.get(`${apiEndpoints.GETIMAGEPRODUCT}/${id}`);
    ...
    return (
        <div>
            <img src={`http://localhost:8000/product_images/${data.image}`} alt={`Gambar ${data.Kode}`} style={{ width: "250px", height: "auto" }} />
        </div>
    );
};
