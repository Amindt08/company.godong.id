...
const GetImageById: React.FC<Props> = ({ id }) => {
    ...
    const fetchDataById = async (id: string) => {
        try {
            const response = await axios.get(`${apiEndpoints.GETIMAGEMKLITE}/${id}`);
    ...
    return (
        <div>
            <img src={`http://localhost:8000/mk_lite_images/${data.image}`} alt={`Gambar ${data.Kode}`} style={{ maxWidth: "100%", height: "auto" }} />
        </div>
    );
};
