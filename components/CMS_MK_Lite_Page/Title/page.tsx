...
const GetTitleById: React.FC<Props> = ({ id }) => {
    ...
    const fetchDataById = async (id: string) => {
        try {
            const response = await axios.get(`${apiEndpoints.GETTITLEMKLITE}/${id}`);
    ...
    if (!data) {
        return <div className="card text-center">
            <ProgressSpinner style={{ width: '20px', height: '20px', zIndex: 'auto' }} strokeWidth="8" fill="var(--surface-ground)" animationDuration=".5s" />
        </div>
    }
